...
function ArticlesOfAmendment() {
    ...
    if (
      ...
      if (feesAdd) {
        ...
        if (data) {
          ...
          dispatch(
            addItem({
              name: `${data?.stateName} Penalty fee`,
              label: `${data?.stateName} Penalty fee`,
              value: `${data?.stateName} Penalty fee`,
              price: data?.penaltyFine,
              id: `${data?.penaltyFine}${data?.stateName}`,
              parentName: `Penalty State`,
            })
          )
          dispatch(addPrice())
        }
      }
      const feesAdd2 = formik.values.entityType?.priceData?.find(
        (e) =>
          e.serviceName.toLowerCase() ===
          "Submit an Annual Report".toLowerCase()
      )

      if (feesAdd2) {
        // Submit an Annual Report
        const data2 = feesAdd2?.fee?.find(
          (e) =>
            e.stateName.toLowerCase() ===
            formik.values.stateFormation?.value?.toLowerCase()
        )
        ...
        if (data2) {
          dispatch(
            addItem({
              name: `${data2?.stateName}  Annual Report fee`,
              label: `${data2?.stateName}  Annual Report fee`,
              value: `${data2?.stateName}  Annual Report fee`,
              price: data2?.price,
              id: `${data2?.price}${data2?.stateName}`,
              parentName: ` Annual Report Address`,
            })
          )
          dispatch(addPrice())
        }
  ...
  }, [formik.values.entityType.value, formik.values.stateFormation.value])
        ...
        <Row>
          <Col lg={8}>
            <div className={styles.textblock}>
              <h1 className={styles.heading}>Reinstatement</h1>
              <p className={styles.tagline}>
                Sometimes life and business can become complex. Staying up to
                date with fees, yearly reports, and taxes are necessary to meet
                the needs of your state and if any of these are neglected, your
                business can be shut down. Filthy Rich Idea can assist you in
                restoring your Good Standing status and resume operations in a
                much shorter period of time and with less cost than it would
                take to set up a new company.
              </p>
            </div>
                  ...
                  <div className={styles.form1}>
                    ...
                    <Select
                      options={entityOptions.filter(
                        (e) =>
                          e.value.toLowerCase().toString() !==
                          "nonprofit".toLowerCase().toString()
                      )}
