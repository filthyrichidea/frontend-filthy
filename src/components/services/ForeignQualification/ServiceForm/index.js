    ...
    if (
        ...
        if (data) {
          dispatch(
            addItem({
              ...
              parentName: `Foreign Qualification State`,
            })
          )
          dispatch(addPrice())
        }
      ...
      const feesAdd2 = formik.values.entityType?.priceData?.find(
        (e) =>
          ...
          "Certificate Of Good Standing".toLowerCase()
      )
      if (feesAdd2) {
        // Certificate of Good qualification
        ...
        if (data2) {
          dispatch(
            addItem({
              name: `${data2?.stateName} fee`,
              label: `${data2?.stateName} fee`,
              value: `${data2?.stateName} fee`,
              price: data2?.price,
          ...
          <Col lg={8}>
            <div className={styles.textblock}>
              ...
              <p className={styles.tagline}>
                A foreign qualification is the process used by a business to
                register with a state where it is not incorporated. This process
                allows the business to conduct activities in that state. It is
                typically used by companies with multi-state operations, such as
                those that sell products or services in several states, or have
                manufacturing and distribution centers in multiple locations.
