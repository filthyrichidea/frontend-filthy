...
function ArticlesOfAmendment() {
  ...
  // console.log(singleService?.standardFees && console.log("true"))
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      exactlyName: Yup.string().required("Required"),
  ...
  useEffect(() => {
    if (
      ...
      const entityData =
        formik.values.entityType?.priceData || formData?.entityType?.priceData
      const feesAdd = entityData?.find(
        (e) =>
        ...
        <Row>
          <Col lg={8}>
            <div className={styles.textblock}>
              <h1 className={styles.heading}>'Doing Business As' Name (DBA)</h1>
              <p className={styles.tagline}>
                A DBA (Doing Business As) is a legal designation that is used to
                register a business name with the state. It is also known as an
                assumed name, fictitious name, or trade name and is typically
                used when a business wishes to operate under a name different
                from its legal name. A DBA allows businesses to conduct business
                under a different name without having to create a new legal
                entity. You can file a DBA online with Filthy Rich Idea in a
                heartbeat!
              </p>
            </div>
                ...
                <form className={styles.form}>
                  ...
                  >
                    ...
                    <div className={styles.formInpWrapper}>
                      <InputField
                        placeholder="Name"
                        value={formik.values.exactlyName}
                        onChange={formik.handleChange}
                        error={formik.errors.exactlyName}
                        name="exactlyName"
                        className={styles.input}
                        autoComplete="off"
                        type="text"
