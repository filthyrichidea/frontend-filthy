...
import { UserFeedBack } from "../../../utils/helperApis"
...
const AdminViewOrder = () => {
  ...
  const adminFormik = useFormik({
    ...
    initialValues: {
      edit: false,
      deleteFile: false,
      adminFile: singleItem?.orderImageUser?.images || null,
    },
    ...
    onSubmit: async (values, action) => {
      ...
      formData.append("orderImageUser[admin]", userData?._id)

      try {
        await UserFeedBack(values, action, params.id, formData)
      } catch (e) {
    ...
    <div>
      <div style={{ padding: "50px 0", background: "#fcfcfc" }}>
        <Container>
          ...
          <DetailCard title="Admin Feedback">
            {!states?.orderMessageAdmin?.title &&
            !states?.orderStatusAdmin?.title ? (
              <p>The file is under reviewing by the admin </p>
            ) : (
              <>
                <div style={{ marginBottom: "10px" }}>
                  <h5> Image</h5>
                  <a
                    href={`${IMAGE_URL}${states?.orderImageAdmin.images}`}
                    rel="noreferrer"
                    target="_blank"
                  >
                    {states?.orderImageAdmin.images}
                  </a>
                </div>

                <div>
                  <h5> Message</h5>
                  <p>{states?.orderMessageAdmin?.title}</p>
                </div>
                <div>
                  <h5> Status Of This Order</h5>
                  <p>{states?.orderStatusAdmin?.title}</p>
                </div>
              </>
            )}
          </DetailCard>

          <DetailCard title="Submit a Document">
      ...
      {states?.serviceId?.title === "Amendment" && <AmendmentForm />}
      {states?.serviceId?.title === "Company Name Change" && <AmendmentForm />}
      ...
      {states?.serviceId?.title === "Get a Trademark" && <TrademarkForm />}

      {states?.serviceId?.title === "Get Reinstated" && <ReinstatementForm />}

      {states?.serviceId?.title === "Submit an Annual Report" && (
        <AnnualReportForm />
      )}
      {!states?.serviceId?.title && <BusinessForm />}
    </div>
  )
}
